...
2. `/check` — on desktops (`isMobileDevice()` heuristic: UA + touch/screen) it shows a QR code (`QrCode.tsx`, offline SVG via `qrcode-generator`) pointing at `{origin}/check` so the user continues on a phone, with an opt-out button for webcam use. On phones it opens the front camera, pre-warms the landmarker, runs a 3‑2‑1 countdown, draws the video frame onto a canvas, and calls `analyzeCapture()`. On success the `AnalysisResult` is stored in `sessionStorage` under `RESULT_STORAGE_KEY` and the page navigates to `/result`. `analyzeCapture()` throws `NoFaceError` (nothing detected) or `FacePartialError` (eye landmarks near the frame edge / eye span < 0.14 — face cropped or too far); the page maps each to its own message.
...
- Purchase leads: every product card has an "order" button opening `OrderModal` (name + phone, honeypot `website` field) → `POST /api/orders`. Each order is its own blob (`orders/{id}.json`, `src/lib/orders.ts`) to avoid concurrent overwrites; the admin "Buyurtmalar" tab lists/toggles/deletes them via `/api/admin/orders`. If `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` env vars are set, new orders also notify Telegram (best-effort, never blocks the order).
